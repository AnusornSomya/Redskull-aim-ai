    ...
    public partial class SettingsMenuControl : UserControl
    {
        ...
        private void LoadModelSettings()
        {
            ...
            builder
                ...
                .AddDropdown("Image Size", d =>
                {
                    ...
                    {
                        var itemValue = (d.DropdownBox.Items[i] as ComboBoxItem)?.Tag?.ToString()
                                        ?? (d.DropdownBox.Items[i] as ComboBoxItem)?.Content?.ToString();
                        if (itemValue == currentSize)
                        {
                    ...
                    d.DropdownBox.SelectionChanged += async (s, e) =>
                    {
                        ...
                        var newSize = (d.DropdownBox.SelectedItem as ComboBoxItem)?.Tag?.ToString()
                                      ?? (d.DropdownBox.SelectedItem as ComboBoxItem)?.Content?.ToString();
                ...
                .AddSlider("AI Minimum Confidence", "% Confidence", 1, 1, 1, 100, s =>
                {
                    ...
                    s.Slider.PreviewMouseLeftButtonUp += (sender, e) =>
                    {
                        ...
                        if (value >= 95)
                            LogManager.Log(LogLevel.Warning, "The minimum confidence you have set for REDSKULL AIM AI to be too high and may be unable to detect players.", true);
                        else if (value <= 35)
                            LogManager.Log(LogLevel.Warning, "The minimum confidence you have set for REDSKULL AIM AI may be too low and can cause false positives.", true);
                    };
        ...
        public void UpdateImageSizeDropdown(string newSize)
        {
            if (_mainWindow?.uiManager.D_ImageSize != null)
            {
                ...
                {
                    var itemValue = (dropdown.DropdownBox.Items[i] as ComboBoxItem)?.Tag?.ToString()
                                    ?? (dropdown.DropdownBox.Items[i] as ComboBoxItem)?.Content?.ToString();
                    if (itemValue == newSize)
                    {
        ...
        private void UpdateTargetClassDropdown(ADropdown dropdown, Dictionary<int, string>? _classes = null)
        {
            ...
            string? selection = (dropdown.DropdownBox.SelectedItem as ComboBoxItem)?.Tag?.ToString()
                                ?? (dropdown.DropdownBox.SelectedItem as ComboBoxItem)?.Content?.ToString();

            var removedItems = dropdown.DropdownBox.Items.Cast<ComboBoxItem>()
                .Where(item => (item.Tag?.ToString() ?? item.Content?.ToString()) != "Best Confidence")
                .ToList();
