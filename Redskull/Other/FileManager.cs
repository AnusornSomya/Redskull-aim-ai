...
namespace Other
{
    internal class FileManager
    {
        ...
        private void CheckForRequiredFolders()
        {
            ...
            {
                MessageBox.Show($"สร้างโฟลเดอร์ที่จำเป็นไม่สำเร็จ: {ex}");
        ...
        private async void ModelListBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            string content = "โมเดลที่โหลด: " + selectedModel;
        ...
        private void ConfigListBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            SelectedConfigNotifier.Content = "คอนฟิกที่โหลด: " + selectedConfig;
        }
                ...
                Application.Current.Dispatcher.Invoke(() =>
                {
                    ...
                    if (ModelListBox.Items.Count > 0)
                    {
                        ...
                        if ((lastLoadedModel == "N/A" || !ModelListBox.Items.Contains(lastLoadedModel)) &&
                            ModelListBox.Items.Contains("1.5kR6.onnx"))
                        {
                            lastLoadedModel = "1.5kR6.onnx";
                        }
                        ...
                        SelectedModelNotifier.Content = $"โมเดลที่โหลด: {lastLoadedModel}";
                    }
                    else
                    {
                        SelectedModelNotifier.Content = "โมเดลที่โหลด: ยังไม่มี";
                    }
                });
            }
        }
            ...
            if (!InQuittingState)
            {
                Application.Current.Dispatcher.Invoke(() =>
                {
                    ...
                    if (ConfigListBox.Items.Count > 0)
                    {
                        ...
                        SelectedConfigNotifier.Content = "คอนฟิกที่โหลด: " + lastLoadedConfig;
                    }
                    else
                    {
                        SelectedConfigNotifier.Content = "คอนฟิกที่โหลด: ยังไม่มี";
                    }
                });
            }
        }
